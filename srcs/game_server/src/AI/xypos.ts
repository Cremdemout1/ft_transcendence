...
// function mse(predicted: number[][], target: number[][]) {
//     let sum = 0;
//     let count = predicted.length;

//     for (let i = 0; i < count; i++) {
//         let dx = predicted[i][0] - target[i][0];
//         let dy = predicted[i][1] - target[i][1];
//         sum += dx*dx + dy*dy;
// 		console.log("predicted: x - "+(predicted[i][0]*50)+", y - "+(predicted[i][1]*50));
// 		console.log("true: x - "+(predicted[i][0]*50)+", y - "+(predicted[i][1]*50));
//     }
...
// function mse_backward(pred: number[][], target: number[][]) {
//     const n = pred.length;
//     let dinputs: number[][] = [];

//     for (let i = 0; i < n; i++) {
//         const dx = 2 * (pred[i][0] - target[i][0]) / n;
//         const dy = 2 * (pred[i][1] - target[i][1]) / n;
//         dinputs.push([dx, dy]);
//     }
//     return dinputs;
// }
...
// function LoadWeights(layer1: Layer_Dense, layer2: Layer_Dense, layer3: Layer_Dense)
// {
// 	const raw = fs.readFileSync("xypos.json", "utf8");
...
// // let layer3 = new Layer_Dense(32, 2, x => x);
...
// // 	if(epoch % 100==0)
// // 		console.log("epoch: "+epoch+", loss: " + mse(layer3.output, samples[1]));
// // 	const dloss = mse_backward(layer3.output, samples[1]);
// // 	layer3.backward(dloss);
...
// // 	layer2.forward(layer1.output);
// // 	layer3.forward(layer2.output);
// // 	let loss= mse(layer3.output, samples[1]);
