...
export class GameMath {
  ...
  movePaddle(paddle: Paddle2D) {
    ...
    if (paddle.up) vertical++;
    if (paddle.down) vertical--;
    if (paddle.left) horizontal--;
    if (paddle.right) horizontal++;
    ...
    if (
      Math.abs(paddle.x) >
      this.gameArea.width / 2 - paddle.height / 2
    ) {
      if (paddle.x > 0)
        paddle.x =
          this.gameArea.width / 2 - paddle.height / 2;
  ...
  private dot_product(x: number, y: number, z: number) {//GET VECTOR MAGNITUDE
    // SQUARE ROOT of dot product of the SAME VECTOR (get magnitude)
    return Math.sqrt(x * x + y * y + z * z);
  }

  private normalize(_x: number, _y: number, _z: number, vel: number) {//NORMALIZE VECTOR
  ...
  private calculateDirection(paddle : Paddle2D) {
    ...
    const offset =
      (this.tmp_ball.y - paddle.y) / (paddle.height / 2);
    ...
    const offsetX =
      (this.tmp_ball.x - paddle.x) / (paddle.height / 2);
