...
export async function createGameScene( //function that makes all the visuals (updates are at the bottom), it takes the engine, the html canvas and the backend calculations object as parameters
  ...
  const player_nbr = await new Promise<number>((resolve) => {
    socket.on("playerCountResponse", ({ numPlayers }: { numPlayers: number }) => {
...
;
  ...
  scene.registerBeforeRender(function () {
    ...
    // Use serverGameState instead of local gameMath
    if (serverGameState && meshes.ball) {
      //updating the position of the ball with the returned values
      meshes.ball.position.set(
        serverGameState.ball.pos.x, //serverGameState returns the updated position of everything
        serverGameState.ball.pos.y,
        serverGameState.ball.pos.z
      );
      
      if (serverGameState.ball.reset) {
    ...
    // Update paddles using serverGameState
   if (serverGameState && meshes.paddles && serverGameState.paddles) {
  meshes.paddles.forEach((paddle, index) => {
    // Add null check for individual paddle and paddle data
    if (paddle && serverGameState.paddles[index]) {
      if (paddle.name == "paddle1" || paddle.name == "paddle5")
        paddle.position.z = serverGameState.paddles[index].x;
      else if (paddle.name == "paddle2" || paddle.name == "paddle6")
        paddle.position.z = -serverGameState.paddles[index].x;
      else if (paddle.name == "paddle3")
        paddle.position.x = -serverGameState.paddles[index].x;
      else paddle.position.x = serverGameState.paddles[index].x;
      ...
      if (
        ...
        paddle.position.y = serverGameState.paddles[index].y;
      else if (paddle.name == "paddle5" || paddle.name == "paddle6")
        paddle.position.x = -serverGameState.paddles[index].y;
    }
  });
}
    if (decal) {
    ...
    if (serverGameState && serverGameState.hit && serverGameState.hitPoint) {
      ...
      if (pickInfo) {
        decal = MeshBuilder.CreateDecal("decal", meshes.arena!, {
          ...
          size: simmetrical_vec((serverGameState.hitPoint.dist)/2),
          cullBackFaces: false,
        });
