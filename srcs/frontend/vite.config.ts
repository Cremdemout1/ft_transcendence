  ...
  server: {
    ...
    proxy: {
      '/api': { //For every request to /api, meaning every call of /api on the frontend/src files
        target: 'http://10.19.238.98:8080', //Forward it to backend server with the right ip, dont need to change every /api call to the right ip
